...
description: >-
  ...
  adds new gap rules, edits gap-detection-rubric.md, syncs SM/APM resume guidelines,
  bumps DIAGNOSIS_PROMPT_VERSION, or tests diagnosis prompt changes.
...
coach-quality gaps (summary positioning, metrics, SM evidence, coaching, AI
readiness, certs, tools, ATS, JD fit, etc.).

## Where to edit (source of truth)

| What | Path |
|------|------|
| **Rubric checklist (production prompt)** | `server/src/diagnosis/gap-detection-rubric.md` |
| **Condensed SM/APM coach guidelines** | `server/src/diagnosis/sm-apm-resume-guidelines-condensed.md` |
...
| Docs pointer | `docs/diagnosis-gap-rubric.md` |

**Upstream founder file (outside repo, not committed):**  
`c:\AgileForum\resume review guidelines- scrum master & Agile project manager.txt`

Comment in `ai-prompt.ts` also points here. Production loads **only**
`gap-detection-rubric.md` via `loadGapDetectionRubric()` into the system prompt.
`npm run build` copies the production `.md` into `dist/diagnosis/`.
The condensed guidelines file is a human/agent reference — do **not** dump it
wholesale into the prompt.

## Sync future guideline edits

When the upstream SM/APM resume guidelines file changes:

1. Distill new must-check rules into `sm-apm-resume-guidelines-condensed.md`
   (keep it condensed; skip exhaustive skill dumps and long ❌/✅ example blocks).
2. Merge actionable checks + Gap example chips into `gap-detection-rubric.md`
   **without removing existing categories** (extend or add numbered sections).
3. Prefer coach must-checks over raw copy — keep prompt size reasonable.
4. Bump `DIAGNOSIS_PROMPT_VERSION` in `ai-diagnosis-schema.ts`.
5. Run `cd server && npm test -- src/diagnosis`.
6. Optional live smoke with a sample SM resume (`AI_PROVIDER_MODE=live`).
...
5. Bump `DIAGNOSIS_PROMPT_VERSION` in `ai-diagnosis-schema.ts` (e.g. `diagnosis-ai-v4` → `v5`) so audit logs show the new prompt.
...
- Prefer Scrum Master / Agile PM coach language from the condensed guidelines.
...
- Do not paste the full upstream skills list into the production rubric.
...
**Mis-positioned summary** — developer/coordinator summary with SM title → expect professional summary gap.

**Ceremony-only** — Daily Scrum/Planning listed, no coaching/metrics/ownership → expect ownership/coaching gaps.

**No AI signal** — modern SM target, zero AI/tools usage → expect AI readiness gap.
...
- Commit or paste the raw external guidelines file into the system prompt.
